...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    for i in range(len(A)):
        ...
        for j in range(len(A[i])):
            row_result.append(A[i][j] + A_trans[i][j])
        A_sum.append(row_result)

    # 1. (A + A^T)x
    solution_A = vanilla_matmul(A_sum, x)

    # 2. y^T B --> B^T y
    B_trans = vanilla_transpose(B)
    solution_B = vanilla_matmul(B_trans, y)

    # 1 + 2
    solution: Vector = [solution_A[i] + solution_B[i] for i in range(len(A))]
...
def numpy_solution(x: np.ndarray, y: np.ndarray, A: np.ndarray, B: np.ndarray) -> np.ndarray:
    ...
    solution = np.matmul(A.T + A, x) + np.matmul(y.T, B)
